...
class LmicroParser {
	...
	const ERR_NUMBER_FORMAT = 29;
	const ERR_BOOLTERM_EXPECTED = 30;
					...
					if ($this->isCapAlpha($mathexp[$i])) { // Handles variables and functions
						...
						if (($i < $length) && $mathexp[$i] == '.') {
							...
							if (($i < $length) && $this->isDigit($mathexp[$i])) {
								...
								$this->errpos = $i;
								$this->error(self::ERR_NUMBER_FORMAT, 'Digit after decimal point missing');
								return false;
							}
						} else {
	...
	private function evaluateNode(array $node) {
		switch ($node['ntype']) {
			...
			case self::N_OROP:
				return ($this->evaluateNode($node['left']) || $this->evaluateNode($node['right']));
			default:
				$this->error(self::ERR_UNKNOWN_NODE_TYPE);
		}
	}
	/**
